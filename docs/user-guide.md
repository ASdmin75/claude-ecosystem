...
- **Pipelines** — список пайплайнов с запуском. В редакторе пайплайна рядом с каждым шагом есть ссылка `↗` для быстрого перехода к соответствующей задаче на странице Tasks
- **Executions** — история исполнений с авто-обновлением
- **Wizard** — генерация конфигураций из текстового описания (MCP-серверы, задачи, пайплайны, домены, агенты)

### Wizard

Wizard позволяет создавать полные конфигурации из текстового описания на естественном языке. За один запрос можно создать:

- **MCP-серверы** (mcp-openapi) — интеграция с внешними API через OpenAPI-спецификацию
- **Домены** — SQLite-базы с автоматической инициализацией схемы и DOMAIN.md
- **Суб-агенты** — специализированные агенты (.md файлы с YAML frontmatter)
- **Задачи** — полные определения с промптами, MCP-серверами, allowed_tools, расписанием
- **Пайплайны** — цепочки задач (sequential/parallel)

**Пример:** для подключения нового внешнего API достаточно:
1. Подготовить OpenAPI-спецификацию (файл `specs/*.yaml`)
2. Добавить API-ключ в `.env`
3. Описать в Wizard что нужно создать — он сгенерирует MCP-сервер, домен, задачи и пайплайн

**Ограничение:** Wizard не создаёт файл OpenAPI-спецификации — он должен существовать на диске до генерации плана.

При применении плана выполняются автоматические проверки и авто-коррекции:
- Все ссылки валидны (задачи → MCP серверы, агенты; пайплайны → задачи)
- `permission_mode: dontAsk` автоматически устанавливается для задач с MCP-инструментами или агентами
- Если задача использует агентов, `"Agent"` автоматически добавляется в `allowed_tools`
- Discovery-инструменты (list_tables, describe_table, list_directory и т.д.) автоматически добавляются в `allowed_tools` для каждого MCP-сервера
- `stop_signal` не должен появляться в промптах не-финальных шагов пайплайна
- После применения генерируется `SETUP.md` с документацией: созданные сущности, статус env-переменных, команды для запуска

#### Troubleshooter (диагностика ошибок)

При ошибках Wizard возвращает структурированный диагноз с категорией и вариантами исправления:

| Категория | Описание | Варианты |
|-----------|----------|----------|
| `empty_output` | Claude вернул пустой ответ | Retry, Retry с контекстом |
| `json_parse` | Невалидный JSON в ответе | Retry, просмотр raw output |
| `timeout` | Превышено время генерации (5 мин) | Retry, упростить описание |
| `duplicate_name` | Имя уже занято | Auto-fix (суффикс `-v2`), ручное редактирование |
| `missing_reference` | Ссылка на несуществующую сущность | Edit Plan, Regenerate |
| `test_soft_failure` | Задача выполнена, но output указывает на проблему | Edit Task, Run Again |

**Retry с контекстом:** при ошибке генерации можно добавить дополнительный контекст — предыдущая ошибка и пояснение пользователя передаются Claude для следующей попытки.

**Редактирование плана:** кнопка "Edit Plan" открывает JSON-редактор. Перед сохранением можно нажать "Validate" для проверки без применения (`POST /wizard/plans/{id}/validate`).

**Test Run:** после применения плана можно запустить тестовый прогон любой созданной задачи (кнопка "Test Run"). Timeout ограничен 2 минутами. Output анализируется на soft failures (permission denied, tool not available, Claude asks for input).
