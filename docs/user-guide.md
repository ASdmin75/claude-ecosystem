...
- **Домен:** автоматически удаляется из `tasks.yaml` когда все его ссылки (tasks, pipelines, agents) очищены в результате удаления. Директория `data_dir` домена сохраняется на диске для ручной очистки.
