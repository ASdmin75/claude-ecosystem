...
Перед удалением всегда создаётся бэкап (снимок `tasks.yaml` + файлы суб-агентов) в `data/backup/{id}/`. Каждая каскадно удалённая сущность также получает свой бэкап-вход с полным снимком конфигурации. Восстановление через API:
