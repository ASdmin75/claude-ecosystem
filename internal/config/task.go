...
// NotifyConfig defines how to send notifications when a task completes.
type NotifyConfig struct {
	Email   []string `yaml:"email,omitempty" json:"email,omitempty"`     // recipient addresses
	Webhook string   `yaml:"webhook,omitempty" json:"webhook,omitempty"` // callback URL
	Trigger string   `yaml:"trigger,omitempty" json:"trigger,omitempty"` // "on_success", "on_failure", "always" (default)
}

// ShouldNotify reports whether a notification should be sent for the given status.
func (n *NotifyConfig) ShouldNotify(status string) bool {
	if n == nil {
		return false
	}
	trigger := n.Trigger
